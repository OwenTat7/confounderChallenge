...
ax.text(105, dreamy_mean, f'○ {dreamy_mean:.2f}', 
        color='#4E79A7', fontsize=11, fontweight='bold',
        verticalalignment='center', horizontalalignment='left')
ax.text(105, duck_mean, f'△ {duck_mean:.2f}', 
...
dreamy_data_randomized = patients_randomized_df[patients_randomized_df['doctor_name'] == 'Doc Dreamy']
duck_data_randomized = patients_randomized_df[patients_randomized_df['doctor_name'] == 'Doc Duck']

# Calculate means
dreamy_mean_randomized = dreamy_data_randomized['post_surgical_score'].mean()
duck_mean_randomized = duck_data_randomized['post_surgical_score'].mean()

# Create figure with professional styling
fig, ax = plt.subplots(figsize=(7, 4))

# Plot scatter points
ax.scatter(dreamy_data_randomized['patient'], dreamy_data_randomized['post_surgical_score'],
           marker='o', color='#4E79A7', s=60, alpha=0.7, 
           label='Doc Dreamy', zorder=3)
ax.scatter(duck_data_randomized['patient'], duck_data_randomized['post_surgical_score'],
           marker='^', color='#E15759', s=60, alpha=0.7,
           label='Doc Duck', zorder=3)

# Add horizontal dashed lines for means
ax.axhline(y=dreamy_mean_randomized, color='#4E79A7', linestyle='--', 
           linewidth=2, alpha=0.8, zorder=2)
ax.axhline(y=duck_mean_randomized, color='#E15759', linestyle='--', 
           linewidth=2, alpha=0.8, zorder=2)

# Add text annotations with shape symbols
ax.text(105, dreamy_mean_randomized, f'○ {dreamy_mean_randomized:.2f}', 
        color='#4E79A7', fontsize=11, fontweight='bold',
        verticalalignment='center', horizontalalignment='left')
ax.text(105, duck_mean_randomized, f'△ {duck_mean_randomized:.2f}', 
        color='#E15759', fontsize=11, fontweight='bold',
        verticalalignment='center', horizontalalignment='left')
...
# Separate observational data by doctor (use patients_df for stratification)
dreamy_data = patients_df[patients_df['doctor_name'] == 'Doc Dreamy']
duck_data = patients_df[patients_df['doctor_name'] == 'Doc Duck']
...
severity_range = max(abs(patients_df['severity'].min()), abs(patients_df['severity'].max()))
x_max = severity_range * 1.15
...
ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
ax.legend(loc='upper right', fontsize=11, framealpha=0.9)
# Set x-axis limits to ensure annotations are visible
ax.set_xlim(-severity_range * 1.2, x_max * 1.1)
